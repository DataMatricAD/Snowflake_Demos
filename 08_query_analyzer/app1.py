...
def getSession():
    section = st.secrets["connections"]["snowflake"]   # <-- matches [connections.snowflake]
    pars = {
        "account": section["account"],
        "user": section["user"],
        "password": section.get("password") or os.environ["SNOWFLAKE_PASSWORD"],
        # optional:
    ...
    pars = {k: v for k, v in pars.items() if v}
